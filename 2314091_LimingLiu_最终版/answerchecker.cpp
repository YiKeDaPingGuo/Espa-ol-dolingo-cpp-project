...
void AnswerChecker::createInputAndCheck(QDialog* dialog, int x, int y, int length, int width, const QString& correctAnswer,bool isnear)
{
//    if (inputBox) {
...
//    }
    ...
    if(isnear){
        checkButton->move(x+length+20, y);
    }else{
        checkButton->move(x,y+width+50);
    }
    checkButton->show();
    ...
    connect(checkButton, &QPushButton::clicked, [=]() {
        ...
        dispCheckAnswer (result,x + length + 70, y);
