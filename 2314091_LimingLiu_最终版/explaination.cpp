...
void Explaination::setWords(QStringList sentences, QWidget *dialog, int x, int y, int size, int spanish, int offset, int setfixedwidth)
{
    ...
    // 创建新按钮
    currentWordsButton = new QPushButton(dialog);
    currentWordsButton->setText(sentence);
    ...
    // 设置按钮为自动填充背景，确保样式表生效
    currentWordsButton->setAutoFillBackground(true);

    // 设置样式表
    currentWordsButton->setStyleSheet(
        "QPushButton {"
        "   border: none;"
        ...
        "   min-height: " + QString::number(size * 2) + "px;" // 设置最小高度以容纳多行文本
        "}"
        "QPushButton:hover {"
        "   background-color: rgba(200, 200, 200, 50);" // 悬停效果
        "}"
    );

    // 设置固定宽度和大小策略
    currentWordsButton->setFixedWidth(setfixedwidth);
    currentWordsButton->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Minimum);

    // 计算并设置按钮高度以适应文本内容
    QFontMetrics fm(font);
    int textHeight = fm.boundingRect(QRect(0, 0, setfixedwidth - 10, 1000), Qt::TextWordWrap, sentence).height();
    currentWordsButton->setFixedHeight(textHeight + 10); // 添加一些额外空间
