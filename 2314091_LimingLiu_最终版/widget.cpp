...
void Widget::dialogThreeFront(std::function<void ()> callback)
{
    ...
    aud3.playSpeak (falstaffChallengeDialog,"qrc:/aud/Falstaff/Falstaff1.wav");
...
void Widget::dialogFourFront(std::function<void ()> callback)
{
    setSentence ("Then Lin has finished all the spanish learning, she would go to the spanish restaurant!",duoChallengeDialog,175,200,20);
    aud4[0].playSpeak (duoChallengeDialog,"qrc:/aud/duo/info.wav");
    setButton ("Genial!",duoChallengeDialog,200,600,300,150,true,[=](){
        QTimer::singleShot(4000, [this,callback]() {
            // 恢复等待动画
            setSentence ("Duo is the servant of the restaurant.He only speak spanish.",duoChallengeDialog,175,200,20);
            qDebug() << "第一段对话结束。";
...
QPushButton* Widget::setChoiceButton(QString filename, int x, int y, int length, int width, QWidget *dialog) {
    ...
    if (QFile::exists(filename)) {
        ...
        if (pix.isNull()) {
            ...
            btn->setIcon(pix.scaled(length, width, Qt::KeepAspectRatio, Qt::SmoothTransformation));
            btn->setIconSize(QSize(length, width));
            btn->setText("");
...
void Widget::ChallengeFour()
{
    if (duoChallengeDialog) {
        isHandlingEventsInMainWidget = false;
        m_currentQuestionIndex4 = 0;
        m_answerResults4.clear();

        dialogFourFront([this](){
...
void Widget::showOscarNextLearn()
{
    ...
    for (int i = 0;i<=6;i++) {
        ...
        aud2[i].playsound (oscarChallengeDialog,db2.findExactRecord (":/csv/alloscaraudio.csv","spanish",m_allChoiceAnswers2[m_currentQuestionIndex2+1][i*2+1])[1],540,350+90*i);
        if(i == 0)continue;
...
void Widget::showOscarNextQuestion()
{
    ...
    for(int i = 0; i < 5; i++){
        ...
        aud2[i].playsound (oscarChallengeDialog,db2.findExactRecord (":/csv/alloscaraudio.csv","spanish",m_allChoiceAnswers2[array1+1][array2*2+1])[1],320,430+80*i);
...
void Widget::showFalstaffNextQuestion()
{
    ...
    sentences.sentenceMaking (m_currentQuestionIndex3,db3);
    QStringList qanda;
    qanda.clear();
...
void Widget::showDuoNextQuestion()
{
    setSentence ("Ah ha!why does lin give up her meal?",duoChallengeDialog,150,400,12);
    setButton ("Le gusta té.",duoChallengeDialog,125,500,450,100,false,[=](){
        setSentence ("Seems that you should practice more.",duoChallengeDialog,150,400,12);
        QTimer::singleShot (4000,[=](){
            onDuoDialogClosed ();
        });

    });
    setButton ("No le gusta el menú.",duoChallengeDialog,125,650,450,100,false,[=](){
        setSentence ("Seems that you should practice more.",duoChallengeDialog,150,400,12);
        QTimer::singleShot (4000,[=](){
            onDuoDialogClosed ();
        });
    });
    setButton ("No tiene bastante dinero.",duoChallengeDialog,125,800,450,100,false,[=](){
        setSentence ("Great! You are the spanish master.",duoChallengeDialog,150,400,12);
        QTimer::singleShot (4000,[=](){
            onDuoDialogClosed ();
        });
    });
...
void Widget::showDuoTest()
{
    if(m_currentQuestionIndex4 == 20){
        showDuoNextQuestion ();
        return;
    }
    m_ImageButtons.append (setChoiceButton (db4.findExactRecord (":/csv/logic.csv","id",QString::number (m_currentQuestionIndex4+1))[1],0,40+(m_currentQuestionIndex4%6)*150,70,70,duoChallengeDialog));
    aud4[m_currentQuestionIndex4].playimage (duoChallengeDialog,db4.findExactRecord (":/csv/logic.csv","id",QString::number (m_currentQuestionIndex4+1))[4],m_ImageButtons.last ());
    ex4[m_currentQuestionIndex4].setWords(db4.findExactRecord (":/csv/logic.csv","id",QString::number (m_currentQuestionIndex4+1)),duoChallengeDialog,80,40+(m_currentQuestionIndex4%6)*150,14,2,1,550);
    setButton ("Go on!",duoChallengeDialog,200,1000,300,150,true,[=](){
        m_currentQuestionIndex4++;
        if(m_currentQuestionIndex4%6 == 0){
            for (int i = 0;i < m_ImageButtons.size ();i++) {
                m_ImageButtons[i]->deleteLater ();
                m_ImageButtons[i] = nullptr;
            }
            for(int i = 0;i<m_currentQuestionIndex4;i++){
                aud4[i].clear ();
                ex4[i].clear ();
            }
        }
...
void Widget::checkFalstaffFinalResult()
{
    ...
    if(!m_answerResults3.empty() && std::all_of(m_answerResults3.begin(), m_answerResults3.end(), [](bool val) { return val; })){
        aud3.playSpeak (falstaffChallengeDialog,"qrc:/aud/Falstaff/Falstaff2.wav");
        setButton ("Finish the study!", falstaffChallengeDialog, 150, 350, 400, 100, true,[this]() {
            QTimer::singleShot(500, [this]() {
                if (falstaffChallengeDialog) {
                    isFalstaffDone = true;
        ...
        aud3.playSpeak (falstaffChallengeDialog,"qrc:/aud/Falstaff/Falstaff3.wav");
        setButton ("Maybe practice again!", falstaffChallengeDialog, 150, 350, 400, 100, true,[this]() {
            QTimer::singleShot(500, [this]() {
                if (falstaffChallengeDialog) {
...
void Widget::onButtonClicked(QPushButton* button)
{
    ...
    if (distance.manhattanLength() <= 100) {
        ...
        if(button == &LilyButton){
            ...
            duoChallengeDialog = (dialog);
            duoChallengeDialog->setWindowTitle("Duo's Challenge");
            duoChallengeDialog->resize(703, 1250);

            if (backgroundPixmap.load(":/png/png/bkground2.png")) {
                QPalette palette = duoChallengeDialog->palette();
                palette.setBrush(QPalette::Window, QBrush(backgroundPixmap.scaled(duoChallengeDialog->size(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation)));
                duoChallengeDialog->setPalette(palette);
                duoChallengeDialog->setAutoFillBackground(true);
            } else {
                qDebug() << "Failed to load background image.";
            }

            connect(duoChallengeDialog, &QDialog::finished, this, &Widget::onDuoDialogClosed);
            dialog->show();
            ChallengeFour();
        }
    }
...
void Widget::onDuoDialogClosed()
{
    ...
    m_currentQuestionIndex4= 0;
    if (duoChallengeDialog) {
        duoChallengeDialog->deleteLater();
        duoChallengeDialog = nullptr;
    }
}
